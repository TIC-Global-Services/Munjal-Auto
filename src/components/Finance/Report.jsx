  ...
  return (
    <div className="bg-black border border-gray-200  p-6 hover:shadow-lg transition-shadow duration-300 rounded-[28px]">
      ...
      <button
          ...
          title="Share report"
        >
          <Share2 size={16} className="text-black" />Share
        </button>
        
      </div>
    

        <h3 className="font-semibold text-[12px] mb-1 mt-4 text-[#C3C1C1]">Annual Report</h3>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-md w-full p-6">
        ...
        <p className="text-gray-600 mb-4">{reportType} - {year}</p>
...
const AnnualReport = () => {
  ...
  const [shareModal, setShareModal] = useState({ isOpen: false, reportType: '', year: '' });
  ...
  const filterBySearch = (reports, searchTerm) => {
    ...
    return reports.filter(report => 
      ...
      // Match individual years like "2023", "2024" etc.
  ...
  const ReportSection = ({ title, years, moreYears, showMore, setShowMore, reportType, filteredReports }) => {
    // Use filtered reports if search is active, otherwise use original logic
