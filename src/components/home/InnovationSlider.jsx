...
const DesktopImageSlider = () => {
  ...
  return (
    <div className="relative w-full my-16 py-20 min-h-[80vh]">
      ...
      <div className="relative flex items-center justify-center h-[50vh] sm:h-[60vh]">
        {images.map((image, index) => (
          ...
          >
            <motion.img
              src={image}
              alt={captions[index]}
              className={`w-[400px] sm:w-[500px] max-w-[80vw] aspect-[4/3] object-cover rounded-xl ${isDragging ? 'cursor-grabbing' : 'cursor-grab'
                }`}
              drag="x"
              dragConstraints={{ left: 0, right: 0 }}
              dragElastic={0.2}
